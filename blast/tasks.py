...
def checkblastruns():
    ...
    for b in BlastRun.objects.filter(deleted=False, server=PASTEUR).filter(Q(status=PENDING) | Q(status=RUNNING)):
        ...
        if state == 'ok':
            ...
            b.state=BlastRun.PENDING
        elif state == 'running':
            b.state=BlastRun.RUNNING
        else:
