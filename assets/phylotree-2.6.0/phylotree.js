(function (global, factory) {
  ...
  function axis(orient, scale) {
    ...
    function axis(context) {
      ...
      path
          .attr("d", (tickSizeOuter ? "M" + range0 + "," + k * tickSizeOuter + "V" + offset + "H" + range1 + "V" + k * tickSizeOuter : "M" + range0 + "," + offset + "H" + range1));
  ...
  function compress(arr, options, matcher) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (property === options.textNodeName) {
        ...
        if (tagObj[":@"]) {
          assignAttributes$1(val, tagObj[":@"], matcher, options);
  ...
  function assignAttributes$1(obj, attrMap, matcher, options) {
    if (attrMap) {
  ...
  function getTaggedNewick(options = {}) {
    ...
    this.display ? this.display._selectionSets : null;
  ...
  */
   ...
   */
  function computeUnrootedLayout(root, scales, width, height, labelPad) {
    labelPad = labelPad || 0;
   ...
   */
  function drawUnrootedEdge(points) {
    return (
   ...
   */
  function unrootedSegmentPlacer(edge, where) {
    return {
  ...
  var selectionSets = /*#__PURE__*/Object.freeze({
    __proto__: null,
  ...
  class TreeRender {
    ...
    update(transitions) {
      ...
      if (this.options["brush"]) {
        ...
        var brush_object = brush_default()
          .on("brush", (event, d) => {
  ...
  ___namespace.extend(TreeRender.prototype, eventEmitter);
  ___namespace.extend(TreeRender.prototype, selectionSets);
  ...
  exports.extractDates = extract_dates;
  ...
  exports.pairwiseDistances = computePairwiseDistances;
