...
presto.translate_value_tree = null;
presto.translate_values_scale = null;
...
d3.layout.phylotree = function(container) {
    var self = new Object,
        ...
        def_branch_length_accessor = function(_node) {
            if ("attribute" in _node && _node["attribute"] && _node["attribute"].length && branch_length_accept == true && typeof _node["__mapped_bl"] === 'undefined') { // MODIFIER ICI POUR SELECTIONNER DENDROGRAMME / PHYLOGRAMME
        ...
        scales = [1, 1],
        fixed_width = [20, 25], // HERE : CHANGER LA TAILLE DE LA FIGURE (ESPACEMENT SURTOUT)
        font_size = 12,
    ...
    phylotree.placenodes = function() {
        ...
        branch_length_accessor_reroot = function(_node) {
            if (branch_length_accept == true) {
                var bl = _node["__mapped_bl"];
                if (!isNaN(bl)) {
                    return Math.max(0, bl);
                }
            }
            ...
            if (a_node['parent']) {
                ...
                if (do_scaling) {
                    ...
                    if (typeof new_json != 'undefined') {
                        a_node.y = branch_length_accessor_reroot(a_node);
                    }
                    else {
                        a_node.y = branch_length_accessor(a_node);
                    }
                    if (typeof a_node.y === 'undefined') {
                        if (a_node["name"] == "__reroot_top_clade") {
                            console.log("bonjour je casse tout");
        ...
        if (draw_scale_bar) { // HERE : changement d'échelle ! 
    ...
    phylotree.handle_node_click = function(node) {
        ...
        if (node) {
            ...
            if (node.parent) {
                ...
                menu_object.append("li").append("a")
                    .attr("tabindex", "-1")
                    .text("Hide " + (d3_phylotree_is_leafnode(node) ? "Node" : "Subtree"))
                    .on("click", function(d) {
                        menu_object.style("display", "none");
    ...
    function subtree_sort_children(n) {
        if (n.children) {
            for (var k = 0; k < n.children.length; k++) {
                subtree_sort_children(n.children[k]);
            }
            n.children.sort(function() {return 1});
        }
    ...
    phylotree.update = function(transitions) {
        ...
        enclosure.attr("transform", function(d) {
            presto.translate_value_tree = [offsets[1] + options["left-offset"], phylotree.pad_height()];
            if (((zoom_value) > 1)) {
                return d3_phylotree_svg_translate([presto.translate_value_tree[0] + (10*zoom_value), presto.translate_value_tree[1] + (10*zoom_value)]) + "scale (" + zoom_value +")" ;
            }
            else {
                return d3_phylotree_svg_translate(presto.translate_value_tree) + "scale (" + zoom_value +")" ;
            }
        });
        ...
        if (draw_scale_bar) {
            ...
            scale_bar.attr("class", css_classes["tree-scale-bar"])
                ...
                .attr("transform", function(d) {
                    presto.translate_value_scale = [offsets[1] + options["left-offset"], phylotree.pad_height() - 10];
                    if (((zoom_value) > 1)) {
                        return d3_phylotree_svg_translate([presto.translate_value_scale[0] + (10*zoom_value), presto.translate_value_scale[1] + (10*zoom_value)]) + "scale (" + zoom_value +")" ;
                    }
                    else {
                        return d3_phylotree_svg_translate(presto.translate_value_scale) + "scale (" + zoom_value +")" ;
                    }
                })
                .call(draw_scale_bar);
...
function d3_phylotree_resize_svg(tree, svg, tr) {
    ...
    if (tree.radial()) {
        ...
        if (svg) {
            presto.translate_value_tree = [pad_radius , (pad_radius + vertical_offset)];
            svg.selectAll("." + tree.css_classes()['tree-container']).attr("transform", "translate (" + presto.translate_value_tree + ") scale (" + zoom_value + ")");
        }
    ...
    if (svg) {
        ...
        if (d3.select("svg").attr("width") < sizes[0]) {
            svg.attr("width", sizes[0]);
        }
        if (d3.select("svg").attr("height") < sizes[1]){
